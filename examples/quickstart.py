...
def simple_filter_example():
    ...
    flow = zephflow.ZephFlow.start_flow().filter("$.value > 10").stdout_sink("JSON_OBJECT")
...
def transformation_example():
  ...
  """
    ...
    # Process events
    events = [{"id": 1, "value": 5}, {"id": 2, "value": 15}, {"id": 3, "value": 25}]
...
def merge_flows_example():
    ...
    high_value_flow = zephflow.ZephFlow.start_flow().filter("$.value >= 20")

    priority_flow = zephflow.ZephFlow.start_flow().filter("$.priority == 'high'")
...
def errors_str(errors):
    ...
    for e in errors:
        str_parts.append(
            f"(error_event: {e.inputEvent().unwrap().toString()}, "
            + f"error_message: {e.errorMessage()})"
        )
    return "[" + ",".join(str_parts) + "]"
