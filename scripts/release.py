...
def run_command(
    cmd: list[str], dry_run: bool = False, capture_output: bool = False
) -> Optional[str]:
    """
...
def verify_version_in_head(
    version_file: Path, python_version: str, java_version: str, dry_run: bool = False
) -> None:
    """
    ...
    head_content = run_command(["git", "show", f"HEAD:{relative_path}"], capture_output=True)
    ...
    if (
        ...
        and f"PYTHON_SDK_VERSION = '{python_version}'" not in head_content
    ):
    ...
    if (
        ...
        and f"JAVA_SDK_VERSION = '{java_version}'" not in head_content
    ):
...
def create_and_push_tag(version: str, tag_message: str, dry_run: bool = False) -> None:
    ...
    run_command(["git", "tag", "-a", tag_name, "-m", tag_message], dry_run=dry_run)
...
def main():
    ...
    if not validate_version_sync(python_version, java_version):
        print("Error: Major.minor versions are not synchronized")
